...
import { ProductAreasEditor, toAreaDrafts, type AreaDraft } from "./ProductAreasEditor";
...
export function AdminProductForm({
  ...
  const [addToPlanners, setAddToPlanners] = useState(!initial);
    ...
    <form onSubmit={handleSubmit} className="max-w-6xl grid md:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        {initial ? (
          <div className="pt-4 border-t border-line">
            ...
            <select
              ...
              className="w-full rounded-lg border border-line px-4 py-3 focus:outline-none focus:border-dark capitalize"
            >
              {STATUSES.map((s) => (
      ...
      <div className="md:col-span-2 xl:col-span-3 rounded-xl border border-line bg-white p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={addVariant} className="text-xs text-terracotta font-medium">
            + Add variant
          </button>
        </div>
        {variants.length === 0 && (
      ...
      <div className="md:col-span-2 xl:col-span-3">
        ...
        >
          {submitting ? "Saving…" : initial ? "Save changes" : "Publish product"}
        </button>
        {!initial && (
